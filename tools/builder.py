...
def dataset_builder_modelnet40():
    train_val_loader = DataLoader(ModelNet40SVM(partition='train', num_points=1024), num_workers=16, batch_size=128,
                                  shuffle=True)
    test_val_loader = DataLoader(ModelNet40SVM(partition='test', num_points=1024), num_workers=16, batch_size=128,
                                 shuffle=True)
...
def build_opti_sche(base_model, config):
    def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
        ...
        for name, param in model.module.named_parameters():
            ...
            if config.optimizer.part == 'ladder':
                if ('cls' in name) or ('adapter' in name) or ('tfts' in name) or ('ori_norm' in name) or ('prompt_pos' in name):
